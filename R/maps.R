maps <- function(){
  ...
  fishing_around_japan <- SAR_data %>% st_as_sf(coords = c("lon","lat"), crs = 4326) %>% 
    ...
    mutate(category = ifelse(matched_category == "unmatched" & fishing_score >= 0.9, "unmatched_fishing",matched_category)) %>%
  ...
  ggsave(mpa_map_1, file = "figures/mpa_map_1.jpg", width = 297, height = 210,units="mm")
  ...
  mpa_map_3 <- SAR_map_3 %>%
    ...
    geom_sf(data = fishing_around_map3, aes(colour = matched_category), size = 0.1, alpha = 0.5) +
    scale_colour_manual(values = c("unmatched"="darkblue",
                                   "matched_fishing"="red"),
                        labels = c("unmatched"="Not publicly tracked",
                                   "matched_fishing"="Publicly tracked")) +
  ...
  ggsave(mpa_map_3, file = "figures/mpa_map_3.jpg", width = 297, height = 210,units="mm")
  ...
  fishing_around_map4 <- SAR_data %>% st_as_sf(coords = c("lon","lat"), crs = 4326) %>% 
    st_crop(c(xmin = 6, xmax = 12, ymin = 40, ymax = 45)) %>%
    #If 90% sure that boat is fishing, it is unmatched and fishing
    mutate(category = ifelse(matched_category == "unmatched" & fishing_score >= 0.8, "unmatched_fishing",matched_category)) %>%
    dplyr::filter(!category %in% c("unmatched","matched_nonfishing","matched_unknown"))
  ...
  mpa_map_4 <- SAR_map_4 %>%
    ...
    geom_sf(data = fishing_around_map4, aes(colour = matched_category), size = 0.1,alpha=0.5) +
    scale_colour_manual(values = c("unmatched"="darkblue",
                                   "matched_fishing"="red"),
                        labels = c("unmatched"="Not publicly tracked",
                                   "matched_fishing"="Publicly tracked")) +
  ...
  fishing_around_map6 <- SAR_data %>% st_as_sf(coords = c("lon","lat"), crs = 4326) %>% 
    st_crop(c(xmin = -120, xmax = -90, ymin = 10, ymax = 23)) %>%
    #If 90% sure that boat is fishing, it is unmatched and fishing
    mutate(category = ifelse(matched_category == "unmatched" & fishing_score >= 0.8, "unmatched_fishing",matched_category)) %>%
    dplyr::filter(!category %in% c("unmatched","matched_nonfishing","matched_unknown"))
  ...
  mpa_map_6<- SAR_map_6 %>%
    ...
    geom_sf(data = fishing_around_map6, aes(colour = matched_category), size = 0.1,alpha=0.5) +
    scale_colour_manual(values = c("unmatched"="darkblue",
                                   "matched_fishing"="red"),
                        labels = c("unmatched"="Not publicly tracked",
                                   "matched_fishing"="Publicly tracked")) +
  ...
  full_maps <- ggarrange(mpa_map_1,mpa_map_2,mpa_map_3,
                         mpa_map_4, mpa_map_5, mpa_map_6, nrow = 3, ncol = 2,common.legend = T )

  ggsave(full_maps, file = "figures/full_maps.jpg", dpi = 300)
